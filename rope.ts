export default class Rope {
	...
	private SPLIT_LEN = 1000;
	private JOIN_LEN = 500;
	private BALANCE_RATIO = 1.2;
	...
	adjust() {
		if (this.str) {
			if (this.str.length > this.SPLIT_LEN) {
				const middle = Math.ceil(this.str.length / 2);
				this.left = new Rope(this.str.substring(0, middle));
				this.right = new Rope(this.str.substring(middle));
				delete this.str;
			}
		} else {
			if (this.length > this.JOIN_LEN) {
	...
	rebalance() {
		if (this.str) {
			if (
				this.left!.length / this.right!.length > this.BALANCE_RATIO ||
				this.right!.length / this.left!.length > this.BALANCE_RATIO
			) {
				this.rebuild();
			} else {
				this.left!.rebalance();
